...
/**
 * Lifted from: https://raw.github.com/mslinn/play21-file-upload-streaming/master/app/services/StreamingBodyParser.scala
 */
object StreamingBodyParser {

  def streamingBodyParser(streamConstructor: String => Option[OutputStream]) = BodyParser {
    request =>
    ...
    // The RequestHeader object is wrapped here so it can be accessed in streamingFilePartHandler
      parse.multipartFormData(
        new StreamingBodyParser(streamConstructor)
  ...
  def streamingFilePartHandler(request: RequestHeader): PartHandler[FilePart[Either[StreamingError, StreamingSuccess]]] = {
    parse.Multipart.handleFilePart {
      case parse.Multipart.FileInfo(partName, filename, contentType) =>
        ...
        fold[Array[Byte], Option[OutputStream]](outputStream) {
          (os, data) =>
            os foreach {
              _.write(data)
            }
